  ...
  do.call(rbind,lapply(modlist,function(i) {
    ...
    #For generalized linear models (family=="poisson") fit using lme4
      ...
      fixef.names=do.call(rbind,lapply(1:length(names(fixef(i))[-1]),function(j) {
        ...
        d[!is.na(d)] } ) )[,1]
      # Generate null model (intercept and random effects only, removed fixed effects)
    ...
    # For model fit using nlme
      ...
      # Get variance of random effects by extracting variance components
      VarRand=sum(suppressWarnings(as.numeric(VarCorr(i)[rownames(VarCorr(i))!="Residual",1])),na.rm=T)
      # Get residual variance
...
# binom.response=rbinom(100,1,0.5)
# mod3=glmer(binom.response~fixed1+fixed2+(1|rand2/rand1),family="binomial",data)
# rsquared.glme(list(mod3))
# #Generalized linear mixed effects model (poisson)
# poisson.response=rpois(100,5)
# mod4=glmer(poisson.response~fixed1+fixed2+(1|rand2/rand1),family="poisson",data)
